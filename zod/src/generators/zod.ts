...
// ── Shared media block (Zod schema + types + runtime helper) ─────────────────
...
function generateSingletonMethods(schema: ManifestSchema): string[] {
  ...
  lines.push(`    const items = this.parse(z.array(${name}Schema), data, '${name}[]');`);
  ...
  lines.push(`    });`);
  lines.push(`    return this.parse(${name}Schema, result, '${name}');`);
  lines.push(`  }`);
...
function generateCollectionMethods(schema: ManifestSchema): string[] {
  ...
  lines.push(`    return this.parse(z.array(${name}Schema), data, '${name}[]');`);
  lines.push(`  }`);
  ...
  lines.push(`    return this.parse(${name}Schema, data, '${name}');`);
  lines.push(`  }`);
...
function generateClient(schemas: ManifestSchema[]): string {
  ...
  lines.push('  }');
  lines.push('');
  lines.push('  private parse<T>(schema: z.ZodType<T>, data: unknown, label: string): T {');
  lines.push('    try {');
  lines.push('      return schema.parse(data);');
  lines.push('    } catch (err) {');
  lines.push('      if (err instanceof z.ZodError) {');
  lines.push('        const issues = err.errors.map(e => `${e.path.join(\'.\')||\'root\'}: ${e.message}`).join(\'; \');');
  lines.push("        throw new Error(`PlatoParseError [${label}]: ${issues}`);");
  lines.push('      }');
...
export function generateZod(manifest: Manifest): string {
  ...
  const sections = [header, '', base];
  if (hasMediaField(manifest.schemas)) sections.push('', MEDIA_BLOCK);
  sections.push('', schemaBlocks, '', client, '');
